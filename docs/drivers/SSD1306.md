...
Gọi **một lần** sau khi peripheral I2C đã được khởi tạo (`MX_I2C1_Init()`). Hàm gửi một mảng lệnh cấu hình duy nhất, mở đầu bằng control byte `0x00`:
...
**Chi phí thời gian.** Với I2C chạy ở 400 kHz (`I2C1_CLOCK_SPEED` trong [main.c](../../stm-firmware/src/main.c)), một lần cập nhật toàn màn hình tốn khoảng **25 ms**:
...
/* Sau khi MX_I2C1_Init() đã chạy xong */
...
SSD1306_UpdateScreen (&hi2c1, &display);
```
...
SSD1306_UpdateScreen (&hi2c1, &display);        /* 3. đẩy lên màn hình */
```
